...
- **`controller/kinematics.py`** — `KinematicChain(model, base_body, ee_site)`: analytical FK for any hinge chain, built once from `MjModel` constants; `fk(qpos)` never reads `MjData`. Validated against MuJoCo at random configurations in `tests/test_kinematics.py`.
