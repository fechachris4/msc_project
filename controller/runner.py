"""Thin two-arm coordinator for the reactive pose-to-position pipeline.

The Runner owns only what is genuinely shared between the arms: one plant
state snapshot per cycle, one target clock, one backend exchange (and so
one simulator step), the combined joint command, and the shared human
safety geometry evaluation.  Reference composition happens upstream
(``arm_flow.py``); the Runner samples the composed source once per cycle
and never rewrites what it sampled.
"""
