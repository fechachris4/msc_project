"""Pure world-frame reactive pose controller.

The controller owns mathematical policy only: world-frame pose/twist error,
PD task twist, damped least-squares IK, and null-space centering.
"""
...
def pose_error(state, target):
    """World-frame pose error, reference minus actual."""
...
def twist_error(state, target):
    """World-frame twist error, reference minus actual."""
    if not isinstance(state, ArmControllerState):
        raise TypeError("state must be an ArmControllerState")
    if not isinstance(target, WorldTarget):
        raise TypeError("target must be a WorldTarget")
...
def solve_reactive_velocity(
    ...
    """Single numerical implementation of PD + DLS + null-space IK."""
